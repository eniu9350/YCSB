...
public class SstoreWorkLoad extends Workload {
	...
	public void init(Properties p) throws WorkloadException {
		...
		fieldlengthgenerator = SstoreWorkLoad.getFieldLengthGenerator(p);
		...
		if (p.getProperty(INSERT_ORDER_PROPERTY, INSERT_ORDER_PROPERTY_DEFAULT)
			...
			keychooser = new ExponentialGenerator(percentile, recordcount
					* frac);
		} else {
			orderedinserts = true;
		}
		...
		if (requestdistrib.compareTo("uniform") == 0) {
			...
			throw new WorkloadException("Unknown request distribution \""
					+ requestdistrib + "\"");
		}
